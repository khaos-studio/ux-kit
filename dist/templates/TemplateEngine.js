...
class TemplateEngine {
     ...
     * @returns The rendered template string
     */
    async renderWithPartials(template, partials, variables) {
        try {
     ...
     */
    renderIterations(template, variables) {
        ...
        return template.replace(iterationRegex, (match, arrayPath, content) => {
            ...
            return arrayValue.map((item, index) => {
                ...
                itemContent = itemContent.replace(/\{\{#unless\s+@last\}\}([\s\S]*?)\{\{\/unless\}\}/g, (_unlessMatch, unlessContent) => {
