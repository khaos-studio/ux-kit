...
class StudyService {
    ...
    async createStudy(name, description = '', projectRoot) {
    ...
    async getStudy(studyId, projectRoot) {
        const studyPath = this.fileSystem.joinPaths(projectRoot, '.uxkit', 'studies', studyId);
    ...
    async deleteStudy(studyId, projectRoot) {
        const studyPath = this.fileSystem.joinPaths(projectRoot, '.uxkit', 'studies', studyId);
        if (!(await this.fileSystem.pathExists(studyPath))) {
    ...
    async generateStudyId(projectRoot, name) {
        const studiesDir = this.fileSystem.joinPaths(projectRoot, '.uxkit', 'studies');
    ...
    async createStudyConfig(studyPath, studyId, name, description) {
        const config = {
            version: '1.0.0',
            studyId,
            name,
    ...
    async createInitialArtifacts(studyPath, studyId, name) {
        // Create questions.md
...
`;
        await this.fileSystem.writeFile(this.fileSystem.joinPaths(studyPath, 'questions.md'), questionsContent);
        // Create sources.md
...
`;
        await this.fileSystem.writeFile(this.fileSystem.joinPaths(studyPath, 'sources.md'), sourcesContent);
        // Create insights.md
...
<!-- Add next steps here -->
`;
        await this.fileSystem.writeFile(this.fileSystem.joinPaths(studyPath, 'insights.md'), insightsContent);
    }
    convertToYaml(obj) {
