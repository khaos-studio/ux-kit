...
class ResearchService {
    constructor(fileSystem, fileGenerator) {
     ...
     */
    async generateQuestions(studyId, prompt, projectRoot) {
        try {
            ...
            const result = await this.fileGenerator.generateQuestions(studyId, studyInfo.name, prompt, projectRoot);
            return {
     ...
     */
    async collectSources(studyId, sources, projectRoot, autoDiscover = false) {
        try {
            // Get study information
            const studyInfo = await this.getStudyInfo(studyId, projectRoot);
            if (!studyInfo) {
            ...
            const result = await this.fileGenerator.generateSources(studyId, studyInfo.name, allSources, projectRoot);
            return {
                success: result.success,
                filePath: result.filePath,
                message: result.message,
     ...
     */
    async summarizeSource(studyId, sourceId, projectRoot) {
        try {
            // Get study information
            const studyInfo = await this.getStudyInfo(studyId, projectRoot);
            if (!studyInfo) {
            ...
            const result = await this.fileGenerator.generateSummary(studyId, sourceId, sourceInfo.title, sourceContent, projectRoot);
            return {
                success: result.success,
                filePath: result.filePath,
                message: result.message,
     ...
     */
    async processInterview(studyId, transcript, participantId, projectRoot) {
        try {
            // Get study information
            const studyInfo = await this.getStudyInfo(studyId, projectRoot);
            if (!studyInfo) {
            ...
            const result = await this.fileGenerator.generateInterview(studyId, participantId, transcript, projectRoot);
            return {
                success: result.success,
                filePath: result.filePath,
                message: result.message,
     ...
     */
    async synthesizeInsights(studyId, projectRoot) {
        try {
            // Get study information
            const studyInfo = await this.getStudyInfo(studyId, projectRoot);
            if (!studyInfo) {
            ...
            const result = await this.fileGenerator.generateInsights(studyId, studyInfo.name, artifacts, projectRoot);
            return {
                success: result.success,
                filePath: result.filePath,
                message: result.message,
    ...
    async getStudyInfo(studyId, projectRoot) {
        try {
    ...
    async getSourceInfo(studyId, sourceId, projectRoot) {
        try {
            const studyPath = this.fileSystem.joinPaths(projectRoot, '.uxkit', 'studies', studyId);
            ...
            for (const line of lines) {
                ...
                if (foundSource && currentSource) {
                    if (line.includes('**Title**:')) {
                        ...
                        if (parts[1])
                            currentSource.title = parts[1].trim();
                    }
                    else if (line.includes('**Type**:')) {
                        const parts = line.split('**Type**:');
                        if (parts[1])
                            currentSource.type = parts[1].trim().toLowerCase();
                    }
                    else if (line.includes('**URL**:')) {
                        const parts = line.split('**URL**:');
                        if (parts[1])
                            currentSource.url = parts[1].trim();
                    }
                    else if (line.includes('**File Path**:')) {
                        const parts = line.split('**File Path**:');
                        if (parts[1])
                            currentSource.filePath = parts[1].trim();
                    }
                }
            }
    ...
    async getSourceContent(sourceInfo, projectRoot) {
        try {
    ...
    async collectResearchArtifacts(studyId, projectRoot) {
        const artifacts = [];
        const studyPath = this.fileSystem.joinPaths(projectRoot, '.uxkit', 'studies', studyId);
        try {
