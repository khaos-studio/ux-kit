...
export class FileGenerator {
   ...
   */
  async autoDiscoverSources(projectRoot: string): Promise<Source[]> {
    ...
    try {
      ...
      for (const file of files) {
        ...
        if (extensions.includes(ext)) {
          const id = `file-${uuidv4().substring(0, 8)}`;
  ...
  private generateQuestionsFromPrompt(prompt: string): string[] {
  ...
  private generateQuestionsContent(studyName: string, studyId: string, questions: string[]): string {
  ...
  private generateSourcesContent(studyName: string, studyId: string, sources: Source[]): string {
    const date = new Date().toISOString().split('T')[0];
  ...
  private generateSummaryFromContent(content: string): { keyTakeaways: string[]; detailedSummary: string } {
  ...
  private generateSummaryContent(sourceTitle: string, sourceId: string, studyId: string, summary: any): string {
    const date = new Date().toISOString().split('T')[0];
  ...
  private processTranscript(transcript: string): { keyThemes: string[]; formattedTranscript: string } {
    // Mock AI implementation
    return {
      keyThemes: [
        'User Experience',
  ...
  private generateInterviewContent(participantId: string, studyId: string, interview: any): string {
    const date = new Date().toISOString().split('T')[0];
    
    let content = `# Interview Transcript: ${participantId}\n\n`;
    content += `**Interview ID**: ${uuidv4().substring(0, 8)}\n`;
    content += `**Study ID**: ${studyId}\n`;
    content += `**Date**: ${date}\n`;
    ...
    interview.keyThemes.forEach((theme: string) => {
      content += `- ${theme}\n`;
    });
  ...
  private synthesizeInsights(artifacts: string[]): { keyFindings: string[]; recommendations: string[]; nextSteps: string[] } {
    // Mock AI implementation
    return {
      keyFindings: [
  ...
  private generateInsightsContent(studyName: string, studyId: string, insights: any): string {
    const date = new Date().toISOString().split('T')[0];
    ...
    insights.recommendations.forEach((recommendation: string) => {
      content += `- ${recommendation}\n`;
    });

    content += `\n## Next Steps\n\n`;
    insights.nextSteps.forEach((step: string) => {
      content += `- ${step}\n`;
    });
