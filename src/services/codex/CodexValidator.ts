...
export class CodexValidator implements ICodexValidator {
  private readonly cliService: ICLIExecutionService;
  private readonly commandTimeout: number = 10000; // 10 seconds
  ...
  async validateCodexCLI(): Promise<CodexValidationResponse> {
    try {
      ...
      const testResult = await this.cliService.executeCommand(
        'codex',
        ['--version'],
        {
